...
static void MEDMEMTest_testMeshAndMeshing()
{
  ...
  const int REFpolyIndex[NumberOfPolyhedron+1] = {1,47,92};
  ...
  medGeometryElement TypesPoly[NbOfTypes] = {MEDMEM_TETRA4, MEDMEM_POLYHEDRA};
  const int NbOfElements[NbOfTypes] = {1,2};
  ...
  medGeometryElement PolygonTypes[NbOfTypes] = {MEDMEM_TRIA3,MEDMEM_POLYGON};
  const int PolygonNumberOfElements[NbOfTypes] = {2,nbOfPolygons};
  ...
  if(PolygonConn)
  {
    ...
    CPPUNIT_ASSERT_EQUAL(MEDMEM_TRIA3,geomPolyElem);
  ...
  for(int ind = SpaceDim; ind > 1; ind-- )
  {
    ...
    if (ind == 2)
    {
      ...
      out << "Max Norm " << maxnorm << " Min Norm " << minnorm << endl;
    ...
    {
      if (ind == 3)
        out << "Barycenter " << i << " " << barycenter->getValueIJ(i,1) << " " << barycenter->getValueIJ(i,2) << " " << barycenter->getValueIJ(i,3) << endl;

      if (ind == 2)
        out << "Barycenter " << i << " " << barycenter->getValueIJ(i,1) << " " << barycenter->getValueIJ(i,2) << endl;
    }
  ...
  // Testing length and normal vectors on 1d elements
  {
    ...
    MED_EN::medGeometryElement EdgeTypes[NumberOfEdgeTypes] = {MEDMEM_SEG2};
    ...
    medGeometryElement Types3[NumberOfTypes3] = {MEDMEM_QUAD4};
    ...
    for (int i = 1; i<=NumberOfElem;i++) {
      ...
      out << "Length " << i << " " << length_value << endl;
      maxlength = dmax(maxlength,length_value);
      minlength = dmin(minlength,length_value);
    }
    out << "Max Length " << maxlength << " Min Length " << minlength << endl;
    ...
    //intersectSupports and mergeSupports methods
    {
      ...
      MED_EN::medGeometryElement gtEdges[1] = {MEDMEM_SEG2};
      int nbEdges1[1] = {1};
      int edges1[1] = {1};
      int nbEdges2[1] = {2};
      int edges2[2] = {2,3};
  ...
  //All cell conn
  meshing->setConnectivity(MED_EN::MED_CELL,MED_EN::MEDMEM_HEXA8,connHexa8);
  //All face conn
  meshing->setConnectivity(MED_EN::MED_FACE,MED_EN::MEDMEM_QUAD4,connQuad4);
  //writing...
  ...
  mesh.addDriver(drv);
  mesh.read();
  const int *conn=mesh.getConnectivity(MED_NODAL,MED_FACE,MEDMEM_ALL_ELEMENTS);
  ...
  //////////////////////////////////////////////////////////
  {
    ...
    //mesh_prof->setConnectivity(connHexa8,MED_EN::MED_CELL,MED_EN::MEDMEM_HEXA8);
    ...
    //mesh_prof->setConnectivity(connQuad4,MED_EN::MED_FACE,MED_EN::MEDMEM_QUAD4);
    mesh_prof->setNumberOfTypes(1,MED_EN::MED_CELL);
    mesh_prof->setTypes(classicalTypesFace,MED_EN::MED_CELL);
    mesh_prof->setNumberOfElements(nbOfFaceElts,MED_EN::MED_CELL);
    mesh_prof->setConnectivity(MED_EN::MED_CELL,MED_EN::MEDMEM_QUAD4,connQuad4);
