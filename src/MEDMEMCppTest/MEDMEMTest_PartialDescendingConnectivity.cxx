...
void MEDMEMTest::testPartialDescendingConnectivity()
{
  ...
  //
  const MED_EN::medGeometryElement classicalTypesEdge[1]={MED_EN::MEDMEM_SEG2};
  ...
  //All cell conn
  myMesh->setConnectivity(MED_EN::MED_CELL,MED_EN::MEDMEM_HEXA8,connNodalCellClassical);
  //All face conn
  myMesh->setConnectivity(MED_EN::MED_FACE,MED_EN::MEDMEM_QUAD4,connNodalFaceClassical);
  //All edge conn
  myMesh->setConnectivity(MED_EN::MED_EDGE,MED_EN::MEDMEM_SEG2,connNodalEdgeClassical);
  //
  /*const int *ConnNodal = */myMesh->getConnectivity(MED_NODAL,MED_CELL,MEDMEM_ALL_ELEMENTS);
  /*const int *ConnNodalIndex = */myMesh->getConnectivityIndex(MED_NODAL,MED_CELL);

  const int *Conn      = myMesh->getConnectivity(MED_DESCENDING,MED_CELL,MEDMEM_ALL_ELEMENTS);
  const int *ConnIndex = myMesh->getConnectivityIndex(MED_DESCENDING,MED_CELL);
  int lgth=myMesh->getConnectivityLength(MED_DESCENDING,MED_CELL,MEDMEM_ALL_ELEMENTS);
  ...
  //
  const int *ConnFaceNodal = myMesh->getConnectivity(MED_NODAL,MED_FACE,MEDMEM_ALL_ELEMENTS);
