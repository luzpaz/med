...
using namespace std;
//CCRTusing namespace MED_FR;
//using namespace MED_EN en include <--> MED_FR ...
...
//CCRT      err = MEDmaaInfo(_medIdt, i ,meshName, &meshDim) ;
      err = MED_FR::MEDmaaInfo(_medIdt, i ,meshName, &meshDim) ;
      if (err != MED_VALID) 
	...
	_ptrMed->_meshes[meshName] = ptrMesh;
      ...
      int index = 0;
//CCRT      for (currentEntity=meshEntities.begin();currentEntity != meshEntities.end(); currentEntity++) {
...
//CCRT	supportName+=entNames[(MED_FR::med_entite_maillage)(*currentEntity).first] ;
	supportName+=MED_FR::entNames[(MED_FR::med_entite_maillage)(*currentEntity).first] ;
...
//CCRT    int                           NbOfGaussPts                 =  0;
    MED_FR::med_int                           NbOfGaussPts                 =  0;
    int                           numberOfTimeSteps            =  -1;
//CCRT    int                           timeStepNumber               =  -1;
    MED_FR::med_int                           timeStepNumber               =  -1;
    //    char                          timeStepUnit[MED_TAILLE_PNOM]= "";
    char                          timeStepUnit[MED_TAILLE_PNOM+1] ;
    double                        timeStep                     = 0.0;
//CCRT    int                           orderNumber                  =  -1;                           //???init?????
    MED_FR::med_int                           orderNumber                  =  -1;                           //???init?????
...
//CCRT    numberOfFields = MEDnChamp(_medIdt,0) ;
...
//CCRT      numberOfComponents = MEDnChamp(_medIdt,i) ;
	  ...
	  for (currentGeometry  = (*currentEntity).second.begin();currentGeometry != (*currentEntity).second.end(); currentGeometry++) {	    
		...
		if ( _meshesIt == _meshes.end() ) {
		  MESSAGE(LOC << "There is no mesh |"
			  ...
			  << fieldName                           <<"|, entity : |"
//CCRT			  << entNames [ (MED_FR::med_entite_maillage)   (*currentEntity).first] <<"|, geometric element of type |" 
			  << MED_FR::entNames [ (MED_FR::med_entite_maillage)   (*currentEntity).first] <<"|, geometric element of type |" 
//CCRT			  << geoNames [ (MED_FR::med_geometrie_element) (*currentGeometry)]     <<"|" 
			  << MED_FR::geoNames [ (MED_FR::med_geometrie_element) (*currentGeometry)]     <<"|" 
			  ); 
		...
		default : {
		  if ( numberOfTimeSteps > 1) 
		    throw MED_EXCEPTION ( LOCALIZED( STRING(LOC) <<
						     " The Field type of |"
						     << fieldName                         <<"|, entity : |"
//CCRT						     << entNames [(MED_FR::med_entite_maillage)   (*currentEntity).first] 
						     << MED_FR::entNames [(MED_FR::med_entite_maillage)   (*currentEntity).first] 
