...
void MeshFuse::expandConnectivity(int final_nb_nodes)
{
  ...
  for ( medEntityMesh entity = MED_CELL; entity < MED_ALL_ENTITIES; ++entity )
  {
    ...
    {
      int max_conn_len = 
        this->getConnectivityLength( nodal, entity, *type) +
       _mesh->getConnectivityLength( nodal, entity, *type);
                                    ...
                                    vector<int> &      elements)
{
  ...
  int sup_type = ( type == MED_POINT1 ? MEDMEM_ALL_ELEMENTS : type );
                                    ...
                                    vector< int > &    old_ids)
{
  ...
  old_conn  =  this->getConnectivity( MED_NODAL, entity, type );
