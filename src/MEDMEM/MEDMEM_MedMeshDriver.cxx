...
const char * const MED_MESH_DRIVER::all_cell_type_tab [MED_N_CELL_GEO_FIXED_CON]=
  { "MEDMEM_POINT1","MEDMEM_SEG2","MEDMEM_SEG3","MEDMEM_SEG4","MEDMEM_TRIA3","MEDMEM_QUAD4","MEDMEM_TRIA6","MEDMEM_TRIA7","MEDMEM_QUAD8","MEDMEM_QUAD9",
    "MEDMEM_TETRA4","MEDMEM_PYRA5","MEDMEM_PENTA6","MEDMEM_HEXA8","MEDMEM_TETRA10","MEDMEM_OCTA12","MEDMEM_PYRA13",
    "MEDMEM_PENTA15","MEDMEM_HEXA20","MEDMEM_HEXA27"};
  ...
  if (_status==MED_OPENED)
    {
      ...
      {
        int nb_cells;
        if(med_2_3::med_geometry_type(*type_iter)!=MED_POLYGON && med_2_3::med_geometry_type(*type_iter)!=MED_POLYHEDRON)
          ...
          nb_cells=med_2_3::MEDmeshnEntity(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,MED_POLYGON,
                                           med_2_3::MED_INDEX_NODE,med_2_3::MED_NODAL,&chgtp3,&trfp3)-1;
        else
          nb_cells=med_2_3::MEDmeshnEntity(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,MED_POLYHEDRON,
      ...
      if (Connectivity->_numberOfTypes > 0)
      {
        ...
        if ( tmp_cell_models.back().getType() == MED_EN::MEDMEM_POLYGON ||
             (!faceTypes.empty() && faceTypes.back() == MED_EN::MEDMEM_POLYGON ))
        {
          polygonConnSize = med_2_3::MEDmeshnEntity(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,MED_POLYGON,med_2_3::MED_CONNECTIVITY,med_2_3::MED_NODAL,&chgtp3,&trfp3);
        }
        if ( tmp_cell_models.back().getType() == MED_EN::MEDMEM_POLYHEDRA )
        {
          polyhedraFacesIndexSize=MEDmeshnEntity(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,MED_POLYHEDRON,med_2_3::MED_INDEX_NODE,med_2_3::MED_NODAL,&chgtp3,&trfp3);
          polyhedraConnSize=MEDmeshnEntity(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,MED_POLYHEDRON,med_2_3::MED_CONNECTIVITY,med_2_3::MED_NODAL,&chgtp3,&trfp3);
        ...
        // bloc to read CELL :
        {
          ...
          for ( i=0;i<Connectivity->_numberOfTypes;i++)
          {
            ...
            case MED_EN::MEDMEM_POLYGON:
              {
                ...
                err2=med_2_3::MEDmeshPolygonRd(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,med_2_3::MED_NODAL,PolygonsConnIndex,tmp_ConnectivityArray);
                if (err2 != MED_VALID)
                {
            ...
            case MED_EN::MEDMEM_POLYHEDRA:
              {
                ...
                err2=med_2_3::MEDmeshPolyhedronRd(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,med_2_3::MED_NODAL,PolyhedronIndex,FacesIndex,tmp_ConnectivityArray);
                if (err2 != MED_VALID)
                {
            ...
            default:
              ...
              int curNbOfElemp3=med_2_3::MEDmeshnEntity(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,med_2_3::med_geometry_type(Connectivity->_geometricTypes[i]),
                                                        med_2_3::MED_CONNECTIVITY,med_2_3::MED_NODAL,&chgtp3,&trfp3);
              char *nomsp3=new char[MED_SNAME_SIZE*curNbOfElemp3+1];
              int *globArrp3=new int[curNbOfElemp3];
              int *famp3=new int[curNbOfElemp3];
              err2=med_2_3::MEDmeshElementRd(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,med_2_3::med_geometry_type(Connectivity->_geometricTypes[i]),
                                             med_2_3::MED_NODAL,med_2_3::MED_FULL_INTERLACE,tmp_ConnectivityArray,&withname,nomsp3,&withnumber,globArrp3,&withfam,famp3);
              delete [] nomsp3;
              delete [] globArrp3;
              delete [] famp3;
              if ( err2 != MED_VALID)
        ...
        if (!faceTypes.empty())
        {
          ...
          for ( i=0; i < (int)faceTypes.size(); i++)
          {
            ...
            if ( faceTypes[i] == MED_EN::MEDMEM_POLYGON )
          ...
          for ( i=0; i<constituent->_numberOfTypes; i++)
          {
            ...
            if ( constituent->_geometricTypes[i] == MED_EN::MEDMEM_POLYGON )
            {
              PointerOf< med_2_3::med_int> PolygonsConnIndex( tmpFaceCount[i]+1 );
              int err2 = med_2_3::MEDmeshPolygonRd(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,med_2_3::MED_NODAL,PolygonsConnIndex,tmp_constituentArray);
            ...
            else
            {
              ...
              med_2_3::med_bool withname=med_2_3::MED_FALSE,withnumber=med_2_3::MED_FALSE,withfam=med_2_3::MED_FALSE;
              int curNbOfElemp3=med_2_3::MEDmeshnEntity(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,med_type,
                                                        med_2_3::MED_CONNECTIVITY,med_2_3::MED_NODAL,&chgtp3,&trfp3);
              char *nomsp3=new char[MED_SNAME_SIZE*curNbOfElemp3+1];
              int *globArrp3=new int[curNbOfElemp3];
              int *famp3=new int[curNbOfElemp3];
              int err=med_2_3::MEDmeshElementRd(_medIdt,_ptrMesh->_name.c_str(),dtp3,itp3,med_2_3::MED_CELL,med_type,
                                                med_2_3::MED_NODAL,med_2_3::MED_FULL_INTERLACE,tmp_constituentArray,&withname,nomsp3,&withnumber,globArrp3,&withfam,famp3);
              delete [] nomsp3;
              delete [] globArrp3;
              delete [] famp3;
...
int  MED_MESH_RDONLY_DRIVER::getFAMILY()
{
  ...
  if (_status==MED_OPENED)
  {
    ...
    if (_ptrMesh->getNumberOfElements( MED_EN::MED_FACE, MED_EN::MEDMEM_ALL_ELEMENTS ))
    {
      // FACE
  ...
  if (err!=0)
    {
      ...
      err = med_2_3::MEDmeshCr(_medIdt,_meshName.c_str(),_ptrMesh->getSpaceDimension(),_ptrMesh->getMeshDimension(),med_2_3::MED_UNSTRUCTURED_MESH,
                               _ptrMesh->_description.c_str(),dtunitp3,med_2_3::MED_SORT_DTIT,
                               rep,const_cast <char *> (tmp_name.c_str()),const_cast <char *> (tmp_unit.c_str()));
            ...
            case MED_EN::MEDMEM_POLYGON:
              {
                ...
                err=med_2_3::MEDmeshPolygonWr(_medIdt,_meshName.c_str(),MED_NO_DT,MED_NO_IT,MED_NO_DT,med_2_3::MED_CELL,med_2_3::MED_NODAL,numberOfElements+1,&tmp_Index[0],&connectivityArray[0]);
            ...
            case MED_EN::MEDMEM_POLYHEDRA:
              {
                ...
                connectivity = ptrMesh->getConnectivity(MED_NODAL, entity, MEDMEM_ALL_ELEMENTS);
                int nbStdCells = ptrMesh->getGlobalNumberingIndex(entity)[i]-1;
                ...
                err=med_2_3::MEDmeshPolyhedronWr(_medIdt,_meshName.c_str(),MED_NO_DT,MED_NO_IT,MED_NO_DT,med_2_3::MED_CELL,med_2_3::MED_NODAL,PolyhedronFacesIndex.size(),
                                                 &PolyhedronFacesIndex[0],PolyhedronIndex.size(),&PolyhedronIndex[0],&PolyhedronConnectivity[0]);
                if (err<0)
      ...
      for (int i=0; i<numberOfTypes; i++)
        {
          ...
          err=med_2_3::MEDmeshElementConnectivityWr(_medIdt,_meshName.c_str(),MED_NO_DT,MED_NO_IT,MED_NO_DT,med_2_3::MED_CELL,(med_2_3::med_geometry_type)types[i],
                                                    med_2_3::MED_DESCENDING,med_2_3::MED_FULL_INTERLACE,numberOfElements,&tmp_Connectivity[0]);
#else
          err=med_2_3::MEDmeshElementConnectivityWr(_medIdt,_meshName.c_str(),MED_NO_DT,MED_NO_IT,MED_NO_DT,med_2_3::MED_CELL,(med_2_3::med_geometry_type)types[i],
                                                    med_2_3::MED_DESCENDING,med_2_3::MED_FULL_INTERLACE,numberOfElements, const_cast<int *>(connectivity));
#endif
...
//
 //mapping elements to all the groups containing it
  ...
  for (int igroup=0; igroup< (int)myGroups.size(); igroup++)
  {
    ...
    {
      for (int ielem = 0; ielem< myGroups[igroup]->getNumberOfElements(MEDMEM_ALL_ELEMENTS); ielem++)
      {
    ...
    {
      const int*Number = myGroups[igroup]->getNumber(MEDMEM_ALL_ELEMENTS);
      for (int ielem = 0; ielem< myGroups[igroup]->getNumberOfElements(MEDMEM_ALL_ELEMENTS); ielem++)
      {
  ...
  {
    if (myGroups[i]->getNumberOfElements(MED_EN::MEDMEM_ALL_ELEMENTS)==0)
    {
...
int MED_MESH_WRONLY_DRIVER::writeFamilyNumbers() const {
  ...
  { // Node related block
    ...
    for (int i=0; i<NumberOfNodesFamilies; i++) {
      ...
      int TotalNumber = (*myFamilies)[i]->getNumberOfElements(MEDMEM_ALL_ELEMENTS);
      if ((*myFamilies)[i]->isOnAllElements())
        for (int j=0; j<TotalNumber; j++)
          MEDArrayNodeFamily[j]=FamilyIdentifier;
      else {
        const int * Number = (*myFamilies)[i]->getNumber(MEDMEM_ALL_ELEMENTS);
        for (int j=0; j<TotalNumber; j++)
  ...
  { // CELLS RELATED BLOCK
        ...
        (((MESH*)_ptrMesh)->existConnectivity(MED_DESCENDING,entity)))
    {
      ...
      // We build the array from the families list objects :
      int NumberOfElements = _ptrMesh->getNumberOfElements(entity, MEDMEM_ALL_ELEMENTS);
  ...
  { // FACE RELATED BLOCK
        ...
        (((MESH*)_ptrMesh)->existConnectivity(MED_DESCENDING,entity)))
    {
      ...
      for (int i=0;i<numberOfFamilies;i++) {
        ...
        int numberOfFamilyElements = (*myFamilies)[i]->getNumberOfElements(MEDMEM_ALL_ELEMENTS);
        if ((*myFamilies)[i]->isOnAllElements())
          for (int ii=0; ii<numberOfFamilyElements; ii++)
            familyArray[ii]=familyNumber;
        else {
          const int * myFamilyElements = (*myFamilies)[i]->getNumber(MEDMEM_ALL_ELEMENTS);
          for (int ii=0;ii<numberOfFamilyElements;ii++)
