      ...
      for (; i < _count[_numberOfTypes]; i++) // for each polygon
      {
        ...
        for (int j=0; j<(int)myPolygon.size()-1; j++) // for each segment of polygon
        {
          ...
          if (!ret_compare)
          {
            ...
            insert_vector(Reversedescendingconnectivityvalue, 2*(NumberOfConstituent+NumberOfNewSeg-1), i+1 + getNumberOf(MED_CELL,MEDMEM_ALL_ELEMENTS)); // add polygon i to reverse descending connectivity for segment_poly (in 1st place)
