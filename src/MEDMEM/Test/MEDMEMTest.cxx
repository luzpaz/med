...
// #42: MEDMEM_Meshing.hxx  }  MEDMEMTest.cxx
 ...
 *   (yetno) MESHING();
 *   (yetno) ~MESHING();
 *   (yetno) void setSpaceDimension (const int SpaceDimension);
 *   (yetno) void setMeshDimension (const int MeshDimension);
 *   (yetno) void setNumberOfNodes (const int NumberOfNodes);
 *   (yetno) void setCoordinates (const int SpaceDimension, const int NumberOfNodes,
 *                                const double * Coordinates,
 *                                const string System, const MED_EN::medModeSwitch Mode);
 *   (yetno) void setCoordinatesSystem(const string System) throw (MEDEXCEPTION);
 *   (yetno) void setCoordinatesNames (const string * names);
 *   (yetno) void setCoordinateName (const string name, const int i);
 *   (yetno) void setCoordinatesUnits (const string * units);
 *   (yetno) void setCoordinateUnit (const string unit, const int i);
 *   (yetno) void setNumberOfTypes (const int NumberOfTypes,
 *                                  const MED_EN::medEntityMesh Entity) throw (MEDEXCEPTION);
 *   (yetno) void setTypes (const MED_EN::medGeometryElement * Types,
 *                          const MED_EN::medEntityMesh Entity) throw (MEDEXCEPTION);
 *   (yetno) void setNumberOfElements (const int * NumberOfElements,
 *                                     const MED_EN::medEntityMesh Entity) throw (MEDEXCEPTION);
 *   (yetno) void setConnectivity (const int * Connectivity, const MED_EN::medEntityMesh Entity,
 *                                 const MED_EN::medGeometryElement Type) throw (MEDEXCEPTION);
 *   (yetno) void setPolygonsConnectivity (const int * ConnectivityIndex, const int * ConnectivityValue,
 *                                         int nbOfPolygons,
 *                                         const MED_EN::medEntityMesh Entity) throw (MEDEXCEPTION);
 *   (yetno) void setPolyhedraConnectivity (const int * PolyhedronIndex, const int * FacesIndex,
 *                                          const int * Nodes, int nbOfPolyhedra,
 *                                          const MED_EN::medEntityMesh Entity) throw (MEDEXCEPTION);
 *   (yetno) void setConnectivities (const int * ConnectivityIndex,
 *                                   const int * ConnectivityValue,
 *                                   const MED_EN::medConnectivity ConnectivityType,
 *                                   const MED_EN::medEntityMesh Entity) throw (MEDEXCEPTION);
 *   (yetno) void addGroup (const GROUP & Group) throw (MEDEXCEPTION);
 *  }
 */
...
// #45: MEDMEM_PointerOf.hxx  }  MEDMEMTest.cxx
 ...
 *   (yetno) PointerOf();
 *   (yetno) ~PointerOf();
 *   (yetno) PointerOf(const int &size);
 *   (yetno) PointerOf(const T *pointer);
 *   (yetno) PointerOf(const int &size, const T *pointer);
 *   (yetno) PointerOf(const PointerOf<T> & pointerOf);
 *   (yetno) PointerOf(const int &size, const PointerOf<T> & pointerOf);
 *   (yetno) operator T*();
 *   (yetno) operator const T*() const;
 *   (yetno) void set(const int &size);
 *   (yetno) void set(const T *pointer);
 *   (yetno) void set(const int &size, const T *pointer);
 *   (yetno) void setShallowAndOwnership(const T *pointer);
 *   (yetno) PointerOf<T>& operator=(const PointerOf<T> &pointer);
 *  }
 *
 *  Use code of MEDMEM/tests/testUPointerOf.cxx
 */
...
// #50: MEDMEM_SkyLineArray.hxx  }  MEDMEMTest.cxx

/*!
 *  Check methods (17), defined in MEDMEM_SkyLineArray.hxx:
 *  class MEDSKYLINEARRAY {
 *   (yetno) MEDSKYLINEARRAY();
 *   (yetno) MEDSKYLINEARRAY(const MEDSKYLINEARRAY &myArray);
 *   (yetno) MEDSKYLINEARRAY(const int count, const int length);
 *   (yetno) MEDSKYLINEARRAY(const int count, const int length,
 *                            const int* index, const int* value, bool shallowCopy=false);
 *   (yetno) ~MEDSKYLINEARRAY();
 *   (yetno) inline int  getNumberOf()       const;
 *   (yetno) inline int  getLength()         const;
 *   (yetno) inline const int*  getIndex()   const;
 *   (yetno) inline const int*  getValue()   const;
 *   (yetno) inline int  getNumberOfI(int i) const throw (MEDEXCEPTION);
 *   (yetno) inline const int*  getI(int i)  const throw (MEDEXCEPTION);
 *   (yetno) inline int  getIJ(int i, int j) const throw (MEDEXCEPTION);
 *   (yetno) inline int  getIndexValue(int i) const throw (MEDEXCEPTION);
 *   (yetno) inline void setIndex(const int* index);
 *   (yetno) inline void setI(const int i, const int* values) throw (MEDEXCEPTION);
 *   (yetno) inline void setIJ(int i, int j, int value) throw (MEDEXCEPTION);
 *   (yetno) inline void setIndexValue(int i, int value) throw (MEDEXCEPTION);
 *   (yetno) friend ostream& operator<<(ostream &os, const MEDSKYLINEARRAY &sky);
 *  }
 *
 *  Use code of MEDMEM/tests/testUSkyLineArray.cxx
 */
