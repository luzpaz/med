                                         ...
                                         MED_EN::medGeometryElement type, int* type_connectivity) const
{
  ...
  for (int i=0; i<m_topology->nbDomain();i++)
  {
    ...
    if (nb_elem>0)
    {
      conn_ip[i]=m_mesh[i]->getConnectivity(MED_EN::MED_NODAL,entity,MED_EN::MEDMEM_ALL_ELEMENTS);
...
/*!gets the connectivity for MEDMEM_POLYGON type
 *
 * \param cell_list list of elements (global cell numbers) for which the connectivity is required
 * \param nb_cells number of elements
 * \param entity type of entity for which the nodal connectivity is required
                                                ...
                                                vector<int>& type_connectivity, vector<int>& connectivity_index) const
{
  ...
  for (int i=0; i<m_topology->nbDomain();i++)
  {
    int nb_elem = m_mesh[i]->getNumberOfElements(entity,MED_EN::MEDMEM_POLYGON);
    if (nb_elem>0)
    {
      conn_ip[i]=m_mesh[i]->getConnectivity(MED_EN::MED_NODAL,entity,MED_EN::MEDMEM_ALL_ELEMENTS);
      conn_index_ip[i] = m_mesh[i]->getConnectivityIndex(MED_EN::MED_NODAL,entity);
...
/*!gets the connectivity for MEDMEM_POLYHEDRA type
 *
 * \param cell_list list of elements (global cell numbers) for which the connectivity is required
 * \param nb_cells number of elements
 * \param entity type of entity for which the nodal connectivity is required
                                                  ...
                                                  vector<int>& type_connectivity, vector<int>& connectivity_index/*, vector<int>& face_connectivity_index*/) const
{
  ...
  for (int i=0; i<m_topology->nbDomain();i++)
  {
    nb_plain_elems[i] = m_mesh[i]->getNumberOfElements(entity, MED_EN::MEDMEM_ALL_ELEMENTS);
    int nb_elem = m_mesh[i]->getNumberOfElements(entity,MED_EN::MEDMEM_POLYHEDRA);
    if (nb_elem>0)
    {
      conn_ip[i]=m_mesh[i]->getConnectivity(MED_EN::MED_NODAL,MED_EN::MED_CELL,MED_EN::MEDMEM_ALL_ELEMENTS);
      conn_index_ip[i] = m_mesh[i]->getConnectivityIndex(MED_EN::MED_NODAL,MED_EN::MED_CELL);
    }
    else
    {
      conn_ip[i]=0;
                                         ...
                                         MED_EN::medGeometryElement type, int* type_connectivity) const
{
  ...
  for (int i=0; i<m_topology->nbDomain();i++)
  {
    int nb_elem = m_mesh[i]->getNumberOfElements(entity,type);
    if (nb_elem>0)
      conn_ip[i]=m_mesh[i]->getConnectivity(
                                            MED_EN::MED_DESCENDING,entity,type);                               
    else
      conn_ip[i]=0;
  }
  ...
  for (int icell=0; icell<nb_cells; icell++)
  {
    ...
    for (int itype=0; itype< number_of_types; itype++)
    {
      if (types[itype].getType() < type)
        type_offset += m_mesh[ip[icell]]->getNumberOfElements(entity,types[itype].getType());
    }
    const int* conn=conn_ip[ip[icell]];
  ...
  for (int idomain=0; idomain<m_topology->nbDomain();idomain++)
    for (int igroup=0; igroup<m_mesh[idomain]->getNumberOfGroups(MED_EN::MED_CELL); igroup++)
      for (unsigned i=0; i<m_indivisible_regions.size(); i++)
      {
        ...
        {
          int nbcells=group->getNumberOfElements(MED_EN::MEDMEM_ALL_ELEMENTS);
          const int* numbers=group->getNumber(MED_EN::MEDMEM_ALL_ELEMENTS);
...
void MESHCollection::buildCellGraph(MEDMEM::MEDSKYLINEARRAY* & array,int *& edgeweights )
{
  ...
  for (int idomain=0; idomain<m_topology->nbDomain(); idomain++)
  {
    ...
    if (nb_cells >0)
    {
      ...
      const int* conn =   m_mesh[idomain]->getConnectivity(
                                                           MED_EN::MED_NODAL,
                                                           MED_EN::MED_CELL,
                                                           MED_EN::MEDMEM_ALL_ELEMENTS);
      int nbnodes = conn_index[nb_cells]-1;
...
void MESHCollection::castSupport(const MESHCollection& old_collection, vector<const MEDMEM::SUPPORT*> old_support, vector<MEDMEM::SUPPORT*> new_support)
{
  ...
  for (int idomain=0; idomain < old_collection.m_topology->nbDomain(); idomain++)
  {
    ...
    int nbelem = support->getNumberOfElements(MED_EN::MEDMEM_ALL_ELEMENTS);
    ...
    if (support->isOnAllElements())
    {
      ...
      list_of_elems = const_cast<int*> (support->getNumber(MED_EN::MEDMEM_ALL_ELEMENTS));
...
void MESHCollection::createNodalConnectivity(const MESHCollection& initial_collection,int idomain, MED_EN::medEntityMesh entity)
{
  ...
  for (iter = type_numbers.begin();iter != type_numbers.end(); iter++)  
  {
    ...
    else if (type == MED_EN::MEDMEM_POLYGON && dimension==2)
    {
    ...
    else if (type == MED_EN::MEDMEM_POLYHEDRA && dimension==3)
    {
  ...
  for (iter = type_numbers.begin();iter != type_numbers.end(); iter++)  
  {
    ...
    if (type != MED_EN::MEDMEM_POLYHEDRA && type != MED_EN::MEDMEM_POLYGON)
    {
                              ...
                              map<MED_EN::medGeometryElement, vector<MEDSPLITTER_FaceModel*> >& face_map)                     
{
  ...
  int* conn = const_cast<int*> (m_mesh[idomain]->getConnectivity(MED_EN::MED_NODAL,constituent_entity, MED_EN::MEDMEM_ALL_ELEMENTS));
  for (int itype=0; itype<nbtypes; itype++)
  {
...
MEDSPLITTER_FaceModel* MESHCollection::getCommonFace(int ip1,int ilocal1,int ip2,int ilocal2,int face_index)
{
  ...
  const int* conn1 = m_mesh[ip1]->getConnectivity(MED_EN::MED_NODAL,MED_EN::MED_CELL,MED_EN::MEDMEM_ALL_ELEMENTS);
  ...
  const int* conn2 = m_mesh[ip2]->getConnectivity(MED_EN::MED_NODAL,MED_EN::MED_CELL,MED_EN::MEDMEM_ALL_ELEMENTS);
