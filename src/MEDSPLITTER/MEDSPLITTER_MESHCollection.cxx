...
MESHCollection::MESHCollection():m_topology(0),m_driver(0),m_driver_type(MEDSPLITTER::MedXML),m_subdomain_boundary_creates(false){}
...
MESHCollection::MESHCollection(const MESHCollection& initial_collection, Topology* topology)
	:m_name(initial_collection.m_name),m_topology(topology),m_owns_topology(false),m_cell_graph(topology->getGraph()),m_driver(0),m_driver_type(MEDSPLITTER::MedXML),m_subdomain_boundary_creates(false)
{
 ...
 */
MESHCollection::MESHCollection(const string& filename):m_topology(0),m_owns_topology(true),m_driver(0),m_driver_type(MEDSPLITTER::Undefined),m_subdomain_boundary_creates(false)
{
 ...
 */
MESHCollection::MESHCollection(const string& filename, const string& meshname):m_name(meshname),m_topology(0),m_owns_topology(true),m_driver(0),m_driver_type(MEDSPLITTER::MedXML),m_subdomain_boundary_creates(false)
{
	char filenamechar[256];
	...
	for (int icell=0; icell<nb_cells; icell++)
		{
			...
			for (int itype=0; itype< number_of_types; itype++)
				{
					if (types[itype].getType() < type) type_offset+=m_mesh[ip[icell]]->getNumberOfElementsWithPoly(entity,types[itype].getType());
				}
			const int* conn=conn_ip[ip[icell]];						
