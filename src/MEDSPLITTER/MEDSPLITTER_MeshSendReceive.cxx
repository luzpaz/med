...
//cout << txt << endl;
  ...
  int deserialize( vector< T >& buffer, vector< TSlice< T > >& data )
  {
    ...
    ptr += max( 1, int(sizeof(int)/sizeof(T)));
  ...
  void makeGroupsAndFamilies ( TMESHING* mesh, vector< TIntSlice >& int_data, int first_elem_slice )
  {
    ...
    {
      MEDMEM::GROUP group( "", fams_of_group[ g ]);
      mesh->addGroup( group );
    }
  }
  ...
  MEDMEM::MESH* makeMesh( vector< TIntSlice >& int_data )
  {
                   ...
                   TDoubleSlice&         coords)
  {
    ...
    if ( mesh->getNumberOfFamilies( MED_CELL ) == 0 && mesh->getNumberOfGroups( MED_CELL ) > 0 ||
         mesh->getNumberOfFamilies(face_entity) == 0 && mesh->getNumberOfGroups(face_entity) > 0 ||
         mesh->getNumberOfFamilies( MED_NODE ) == 0 && mesh->getNumberOfGroups( MED_NODE) > 0 )
      mesh->createFamilies();
                           ...
                           const vector<int>& node_glob_numbers)
{
  ...
  if ( _mesh && getMeshData( _mesh, int_data, char_data, coords ))
  {
    ...
    MPI_Isend((void*)(double*)coords._pointer, coords._size, MPI_DOUBLE, irank,
...
//   MEDMEM::STRING out("Receive dom ");
//   out << idomain << " from proc " << irank << " - " <<
//     _mesh->getNumberOfElements(MED_EN::MED_CELL, MED_EN::MED_ALL_ELEMENTS) << " cells";
//   cout << out << endl;
...
MeshSendReceive() {}
~MeshSendReceive() {}
void send(int irank, int idomain, MEDMEM::MESH* mesh, const vector<int>& cell_glob_nums, const vector<int>& face_glob_nums, const vector<int>& node_glob_numbers) {}
MEDMEM::MESH* recv(int irank,int idomain, vector<int>& cell_glob_nums, vector<int>& face_glob_nums, vector<int>& node_glob_numbers) { return 0; }
MEDMEM::MESH* getMesh() { return 0; }
bool isSent() { return 0; }
void clear() {}
