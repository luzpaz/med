...
MEDCalculatorBrowserField::MEDCalculatorBrowserField(const char *fname, const char *fieldName) : _name(fieldName), _file_name(fname), _selection(false)
{
  ...
  std::vector<TypeOfField> types=MEDLoader::GetTypesOfField(fname,fieldName,meshNames[0].c_str());
  _type=types[0];//To improve
  MEDCouplingFieldDouble *tmpf=MEDLoader::ReadField(_type,fname,meshNames[0].c_str(),0,fieldName,dtits[0].first.first,dtits[0].first.second);
