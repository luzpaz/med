...
void MEDLoader::CheckFileForRead(const char *fileName) throw(INTERP_KERNEL::Exception)
{
  ...
  int fid=MEDfileOpen(fileName,MED_ACC_RDONLY);
  if(fid<0)
    {
  ...
  for(int i=0;i<nfam;i++)
    {
      ...
      char *gro=new char[MED_LNAME_SIZE*ngro+1];
      MEDfamilyInfo(fid,meshName,i+1,nomfam,&numfam,gro);
  ...
  for(int i=0;i<nbFields;i++)
    {
      ...
      char *comp=new char[ncomp*MED_SNAME_SIZE+1];
      char *unit=new char[ncomp*MED_SNAME_SIZE+1];
      char *dt_unit=new char[MED_LNAME_SIZE+1];
      med_int nbPdt;
...
void MEDLoaderNS::writeUMeshesDirectly(const char *fileName, const std::vector<const ParaMEDMEM::MEDCouplingUMesh *>& mesh, const std::vector<const DataArrayInt *>& families, bool forceFromScratch, bool &isRenumbering)
{
  ...
  char *desc=MEDLoaderBase::buildEmptyString(MED_COMMENT_SIZE);
  MEDLoaderBase::safeStrCpy(meshName.c_str(),MED_NAME_SIZE,maa,MEDLoader::_TOO_LONG_STR);
  MEDLoaderBase::safeStrCpy(meshName.c_str(),MED_COMMENT_SIZE,desc,MEDLoader::_TOO_LONG_STR);
  const int spaceDim=mesh[0]->getSpaceDimension();
  const int meshDim=mesh[0]->getMeshDimension();
  DataArrayDouble *arr=mesh[0]->getCoords();
  char *comp=MEDLoaderBase::buildEmptyString(spaceDim*MED_SNAME_SIZE);
  char *unit=MEDLoaderBase::buildEmptyString(spaceDim*MED_SNAME_SIZE);
  for(int i=0;i<spaceDim;i++)
    {
      std::string info=arr->getInfoOnComponent(i);
      std::string c,u;
      MEDLoaderBase::splitIntoNameAndUnit(info,c,u);
      MEDLoaderBase::safeStrCpy(c.c_str(),MED_SNAME_SIZE-1,comp+i*MED_SNAME_SIZE,MEDLoader::_TOO_LONG_STR);//MED_TAILLE_PNOM-1 to avoid to write '\0' on next compo
      MEDLoaderBase::safeStrCpy(u.c_str(),MED_SNAME_SIZE-1,unit+i*MED_SNAME_SIZE,MEDLoader::_TOO_LONG_STR);//MED_TAILLE_PNOM-1 to avoid to write '\0' on next compo
    }
  ...
  for(std::set<int>::const_iterator it=familyIds.begin();it!=familyIds.end();it++,fid++)
    {
      ...
      for(int i=0;i<ngro;i++)
        MEDLoaderBase::safeStrCpy(meshes[gidsOfFamilies[fid][i]]->getName(),MED_LNAME_SIZE-1,groName+i*MED_LNAME_SIZE,MEDLoader::_TOO_LONG_STR);//MED_LNAME_SIZE-1 to avoid to write '\0' on next compo
...
void MEDLoaderNS::appendNodeProfileField(const char *fileName, const ParaMEDMEM::MEDCouplingFieldDouble *f, const int *thisMeshNodeIds)
{
  med_int numdt,numo;
  med_float dt;
  char *nommaa=MEDLoaderBase::buildEmptyString(MED_NAME_SIZE);
  ...
  char *profileName=MEDLoaderBase::buildEmptyString(MED_NAME_SIZE);
...
med_idt MEDLoaderNS::appendFieldSimpleAtt(const char *fileName, const ParaMEDMEM::MEDCouplingFieldDouble *f, med_int& numdt, med_int& numo, med_float& dt)
{
  ...
  int nbComp=f->getNumberOfComponents();
  char *comp=MEDLoaderBase::buildEmptyString(nbComp*MED_SNAME_SIZE);
  char *unit=MEDLoaderBase::buildEmptyString(nbComp*MED_SNAME_SIZE);
    ...
    case ParaMEDMEM::ON_GAUSS_PT:
      {
        ...
        for(std::list<MEDLoader::MEDFieldDoublePerCellType>::const_iterator iter=split.begin();iter!=split.end();iter++)
          {
            char *nomGauss=MEDLoaderBase::buildEmptyString(MED_NAME_SIZE);
