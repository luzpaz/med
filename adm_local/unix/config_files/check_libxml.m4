...
AC_MSG_RESULT(\$LIBXMLDIR = ${LIBXMLDIR})

CPPFLAGS_old="${CPPFLAGS}"
LIBS_old=$LIBS

if test "x${LIBXMLDIR}" != "x" ; then
  LIBXML_CPPFLAGS="-I${LIBXMLDIR}/include/libxml2"
  LIBXML_LIBS="-L${LIBXMLDIR}/lib -lxml2"
fi
...
if test "x${LIBXMLDIR}" != "x" ; then
  AC_CHECK_FILE(${LIBXMLDIR}/include/libxml2/libxml/tree.h,
                libxml_include_dir_ok=yes,
...
dnl LIBXML binaries
  ...
  if test "x${LIBXMLDIR}" != "x" ; then
    AC_CHECK_FILE(${LIBXMLDIR}/lib/libxml2.a,
                  libxml_lib_dir_ok=yes,
